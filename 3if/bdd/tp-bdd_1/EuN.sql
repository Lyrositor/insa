...
/*
ALTER TABLE COMMANDES_EuN
  ADD CONSTRAINT FK_COMMANDES_EuN_EMPLOYES
  FOREIGN KEY (NO_EMPLOYE) REFERENCES EMPLOYES (NO_EMPLOYE);
=> FAIT

ALTER TABLE COMMANDES_O
  ADD CONSTRAINT FK_COMMANDES_O_EMPLOYES
  FOREIGN KEY (NO_EMPLOYE) REFERENCES EMPLOYES (NO_EMPLOYE);
=> FAIT

ALTER TABLE DETAILS_COMMANDES_EuN
  ADD CONSTRAINT FK_DETAILS_COMMANDES_EuN_PRODUITS
  FOREIGN KEY (REF_PRODUIT) REFERENCES PRODUITS (REF_PRODUIT);

ALTER TABLE DETAILS_COMMANDES_O
  ADD CONSTRAINT FK_DETAILS_COMMANDES_O_PRODUITS
  FOREIGN KEY (REF_PRODUIT) REFERENCES PRODUITS (REF_PRODUIT);
*/
  
/* FOREIGN KEY REMOTE */
/* FOREIGN KEY REMOTE INSERT */
CREATE OR REPLACE TRIGGER TR_FK_COMMANDES_EuN_EMPLOYES
BEFORE INSERT OR UPDATE ON COMMANDES_EuN
FOR EACH ROW
DECLARE
  nbEmp int;
BEGIN
...
CREATE OR REPLACE TRIGGER TR_FK_COMMANDES_O_EMPLOYES
...
CREATE OR REPLACE TRIGGER TR_FK_COMMANDES_EuN_CLIENT
BEFORE INSERT OR UPDATE ON COMMANDES_EuN
FOR EACH ROW
DECLARE
  nbClient int;
BEGIN
  SELECT COUNT(*) INTO nbClient FROM CLIENT WHERE CODE_CLIENT=:NEW.CODE_CLIENT;
  IF (nbClient=0) THEN
    RAISE_APPLICATION_ERROR(-20002, 'CLIENT INCONNU');
  END IF;
END;

CREATE OR REPLACE TRIGGER TR_FK_COMMANDES_O_CLIENT
BEFORE INSERT OR UPDATE ON COMMANDES_O
FOR EACH ROW
DECLARE
  nbClient int;
...
CREATE OR REPLACE TRIGGER TR_FK_DETAILS_COMMANDES_EuN_COMMANDES
BEFORE INSERT OR UPDATE ON DETAILS_COMMANDES_EuN
FOR EACH ROW
DECLARE
  nbCommande int;
BEGIN
  SELECT COUNT(*) INTO nbCommande FROM COMMANDES WHERE NO_COMMANDE=:NEW.NO_COMMANDE;
  IF (nbCommande=0) THEN
    RAISE_APPLICATION_ERROR(-20002, 'COMMANDE INCONNU');
  END IF;
END;

CREATE OR REPLACE TRIGGER TR_FK_DETAILS_COMMANDES_O_COMMANDES
BEFORE INSERT OR UPDATE ON DETAILS_COMMANDES_O
FOR EACH ROW
DECLARE
  nbCommande int;
BEGIN
...
CREATE OR REPLACE TRIGGER TR_FK_DETAILS_COMMANDES_EuN_PRODUITS
BEFORE INSERT OR UPDATE ON DETAILS_COMMANDES_EuN
FOR EACH ROW
DECLARE
  nbProduit int;
BEGIN
  SELECT COUNT(*) INTO nbProduit FROM PRODUITS WHERE REF_PRODUIT=:NEW.REF_PRODUIT;
  IF (nbProduit=0) THEN
    RAISE_APPLICATION_ERROR(-20002, 'PRODUIT INCONNU');
  END IF;
END;

CREATE OR REPLACE TRIGGER TR_FK_DETAILS_COMMANDES_O_PRODUITS
BEFORE INSERT OR UPDATE ON DETAILS_COMMANDES_O
FOR EACH ROW
DECLARE
  nbProduit int;
...
DECLARE
  nbFour int;
BEGIN
  SELECT COUNT(*) INTO nbFour FROM AFAVIER.FOURNISSEURS WHERE NO_FOURNISSEUR=:NEW.NO_FOURNISSEUR;
  IF (nbFour=0) THEN
...
CREATE OR REPLACE TRIGGER TR_FK_PRODUITS_AUTRES
...
DECLARE
  ...
  nbProdS_A int;
BEGIN
  SELECT COUNT(*) INTO nbProdDC_EUN FROM AFAVIER.DETAILS_COMMANDES_EUN@EuN WHERE REF_PRODUIT=:NEW.REF_PRODUIT;
  SELECT COUNT(*) INTO nbProdDC_O FROM AFAVIER.DETAILS_COMMANDES_O@EuN WHERE REF_PRODUIT=:NEW.REF_PRODUIT;
  SELECT COUNT(*) INTO nbProdDC_A FROM OSTEIN.DETAILS_COMMANDES_A@Am WHERE REF_PRODUIT=:NEW.REF_PRODUIT;
  
  SELECT COUNT(*) INTO nbProdS_EUN FROM AFAVIER.STOCK_EUN@EuN WHERE REF_PRODUIT=:NEW.REF_PRODUIT;
  SELECT COUNT(*) INTO nbProdS_O FROM AFAVIER.STOCK_O@EuN WHERE REF_PRODUIT=:NEW.REF_PRODUIT;
  SELECT COUNT(*) INTO nbProdS_A FROM OSTEIN.STOCK_A@Am WHERE REF_PRODUIT=:NEW.REF_PRODUIT;
  
  IF ((nbProdDC_EUN + nbProdDC_O + nbProdDC_A + nbProdS_EUN + nbProdS_O + nbProdS_A)>0) THEN
    RAISE_APPLICATION_ERROR(-20003, 'PRODUIT ENCORE UTILISE');
  END IF;
END;
